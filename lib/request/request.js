...
export default new class {
    ...
    async post(url, options = {}) {
        ...
        if (options.data) {
            logger.debug("[止水插件]POST request params data: ", options.data)
    ...
    getAgent(cf, isProxy = true) {
        let { proxyAddress, cfTLSVersion } = Config.proxy
        return cf
    ...
    async proxyRequestImg(url, { cache, timeout, headers } = {}) {
        ...
        logger.debug(`[止水插件][proxyRequestImg][${_.truncate(url)}] ${kb} ${ms}`)
    ...
    _prepareRequest(options) {
        ...
        logger.debug(`[止水插件] ${options.method.toUpperCase()}请求：${decodeURI(options.url)}`)
        options.headers = {
