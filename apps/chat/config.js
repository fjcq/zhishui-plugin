    ...
    try {
        ...
        merged['好感度管理'] = {
            ...
            '操作指南': supportsToolCalling ? [
                '每次对话结束后，主动评估用户的言行是否影响好感度',
                '如果用户的行为触发了好感度变化，必须调用set_user_favor工具更新',
                '当需要了解用户好感度时，必须调用get_user_favor工具获取真实数据',
                '不要编造好感度数值，必须通过工具获取真实数据',
                '好感度调整应该是自然、渐进的，不要频繁大幅调整'
            ] : [
                '从用户信息中感知当前好感度状态',
                '根据对话内容自然地调整好感度',
                '好感度变化会自动记录，无需向用户说明',
            ...
            '调整触发条件': {
                '增加好感度': {
                    '轻微(+1~+3)': ['礼貌问候', '真诚关心', '有趣玩笑'],
                    '中度(+4~+6)': ['真诚赞美', '主动帮助', '理解角色感受'],
                    '大幅(+7~+10)': ['困难时支持', '深度理解', '情感共鸣']
                },
                '降低好感度': {
                    '轻微(-1~-3)': ['无礼言语', '无视感受', '无聊话题'],
                    '中度(-4~-6)': ['明显冒犯', '欺骗隐瞒', '嘲讽贬低'],
                    '大幅(-7~-10)': ['严重伤害', '背叛信任', '恶意攻击']
                },
                '保持不变': ['普通查询', '中性闲聊', '无明显情感倾向']
            }
        };
