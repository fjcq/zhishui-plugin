 ...
 *
 * 【重要说明】
 * 本文件为早期设计的"单一数据源"注册表，但实际未被任何模块 import 使用：
 * - AI 实际看到的工具定义来自 definitions/*.js（通过 definitions/index.js 聚合）
 * - 锅巴面板的工具开关配置来自 guoba/schemas/toolSwitchSchema.js
 * - 本文件中的 definition 字段（含部分工具的旧参数定义，如 generate_meme 的 meme_type enum）
 *   与 definitions/*.js 中的最新定义可能不同步，请勿以本文件为准
 *
 * 保留本文件仅为兼容性考虑，后续如有需要可安全删除。
 * 修改工具定义请直接编辑 definitions/*.js。
 */

/**
 * 工具分类定义（仅本文件内部使用，锅巴面板实际使用 toolSwitchSchema.js 中的定义）
 */
export const CATEGORIES = [
    ...
    { id: 'image', label: '生图工具', field: 'image_tools' }
];
...
【双头像表情】kiss和rub需要两个用户头像，可使用user_id_2指定第二个用户，不填则默认使用你的头像`,
            parameters: {
                type: "object",
                properties: {
                    meme_type: { type: "string", description: "表情包类型", enum: [
...
export function migrateToolConfig(oldConfig) {
    ...
    const TOOL_CATEGORY_FIELDS = [
        ...
        'search_tools', 'video_tools', 'image_tools'
    ];
