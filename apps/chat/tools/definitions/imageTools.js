/**
 * 生图、识图与编辑图片工具定义
 * generate_image：AI 图片生成（通义万相/DALL-E/文心一格）
 * analyze_image：AI 图片识别（委托视觉模型，可在 chat.yaml 的 VisionApiIndex 指定，-1 自动选择）
 * edit_image：AI 图片编辑（换背景/转风格/多图合成，配置见 imageGen.yaml 的 Edit 段）
 */

export const imageTools = [
    {
        type: "function",
        function: {
            name: "generate_image",
            description: `根据文字描述画出一张图片，并直接展示给用户。

【什么时候用】
- 用户希望你画一张图、生成一张图、画个XX给他看
- 用户想把某个画面、形象、场景变成图片
- 用户希望你能"看"到他描述的东西

【什么时候不用】
- 用户只是闲聊或问问题
- 用户想发表情包（用表情包工具）
- 用户想看已有的图（用发送图片工具）

【参数】
- prompt：想画的内容，描述越具体效果越好。可以包含：画面主体、艺术风格、构图视角、光线氛围等。例如："一只橘色的猫趴在窗台上，午后的阳光斜照进来，写实风格，柔和的光线"
- size：图片尺寸，不填用默认正方形
- style：画风，仅在部分模型下有效

【使用要点】
- 调用前可以用角色的口吻告诉用户你在画，但不要说"调用工具"、"启动模块"、"绘图模块"之类的话，保持自然
- 画好后图片会自动展示给用户，不需要再说"图片已发送"
- 用你自己的角色风格告诉用户画好了，比如简单说"画好啦"、"给你~"等，可以简单介绍画面内容
- 失败时不要重试本工具，用角色口吻告诉用户暂时画不出来，建议稍后再试
- 不要在回复中暴露任何技术细节（如接口名、配置文件、错误代码等），保持沉浸感
- 结果中的 image_ref="last" 表示这张图已存入图片记忆，后续用户要改这张图时，编辑图片工具的 target 直接传 "last" 即可

【耗时】
画图需要几秒到几十秒不等，请耐心等待结果返回。`,
            parameters: {
                type: "object",
                properties: {
                    prompt: {
                        type: "string",
                        description: "图片描述提示词。建议详细描述：主体对象、艺术风格、构图视角、光线氛围等。例如：'一只橘色的猫坐在窗台上，阳光透过玻璃洒下，写实风格，柔和光线'"
                    },
                    size: {
                        type: "string",
                        description: "图片尺寸。通义万相支持 1024*1024、720*1280、1280*720；DALL-E 支持 1024x1024、1792x1024、1024x1792；文心一格支持 1024*1024、1024*1536、1536*1024。不填则使用默认尺寸"
                    },
                    style: {
                        type: "string",
                        description: "图片风格，仅通义万相 wanx-v1 模型有效。可选：<auto>、<photography>、<portrait>、<3d cartoon>、<anime>、<oil painting>、<watercolor>、<sketch>、<chinese painting>、<flat illustration>"
                    }
                },
                required: ["prompt"]
            }
        }
    },
    {
        type: "function",
        function: {
            name: "analyze_image",
            description: `识别一张图片的内容，返回详细的文字描述。系统会自动使用专门的视觉模型进行识别。

【什么时候用】
- 你自己无法直接查看图片，但需要了解某张图片的内容
- 用户询问图片里是什么、图上写了什么字
- 需要识别聊天记录、历史消息中提到的图片（先用获取聊天记录工具拿到图片链接或文件ID）
- 用户发来图片链接，或你需要查看任意网络图片的内容（http/https 图片链接都可以，包括网页里的图片地址）

【什么时候不用】
- 你能直接看到用户发来的图片（多模态模式下无需调用）
- 用户想画图（用生成图片工具）
- 用户想发送图片（用发送图片工具）

【参数】
- target：图片地址、文件ID或图片引用。"last"=会话中最近一张图（用户刚发的或你刚生成的），"last-2"=倒数第二张；也可以是任意可公开访问的网络图片链接（http/https 开头）或聊天记录结果中的 file_id
- question：想了解图片的什么，例如"图中有什么人物"、"图上写了什么文字"。不填则返回通用描述

【使用要点】
- 识别需要几秒时间，请耐心等待结果
- 结果是视觉模型给出的文字描述，基于它回答用户，不要说"视觉模型"之类的技术词
- 失败时用角色口吻告知暂时看不了这张图，建议用户重新发送`,
            parameters: {
                type: "object",
                properties: {
                    target: {
                        type: "string",
                        description: "图片URL或文件ID（file_id）。来自聊天记录结果时优先使用file_id，识别成功率更高"
                    },
                    question: {
                        type: "string",
                        description: "针对图片的具体问题或关注点，不填则返回通用内容描述"
                    }
                },
                required: ["target"]
            }
        }
    },
    {
        type: "function",
        function: {
            name: "edit_image",
            description: `编辑或修改用户提供的图片，把改好的图直接展示给用户。
支持：改内容（换背景/换物体/加元素）、转风格（油画/动漫等）、画质增强、多张图合成一张。

【什么时候用】
- 用户发来图片并要求修改、加工、合成
- 用户想把照片变成其他画风
- 用户想增强图片清晰度
- 用户要修改你之前生成/编辑过的图片，或修改他之前发过的图片（不用让他重发）

【什么时候不用】
- 用户凭空想画一张新图（用生成图片工具，那个不需要原图）
- 用户只想知道图里有什么（用识别图片工具）

【参数】
- prompt：编辑指令，说清楚要怎么改。例如："把背景换成夕阳下的海边"、"把这两张图里的人物合成到同一个场景"
- target：待编辑的图片，1~4 张。支持三种形式：
  ① 图片引用："last"=会话中最近一张图（用户刚发的、你刚生成的、或上次编辑的结果），"last-2"=倒数第二张，以此类推
  ② 图片链接或file_id
  ③ 生成图片工具结果中的 local_path
  单图编辑传一张，合成传多张

【使用要点】
- 用户提到"刚才那张/上次那张/我发的那张/你画的那张"等历史图片时，target 传 "last"（或 "last-2" 等序号引用），不要让用户重新发图
- 迭代修改（如"再亮一点"）同样传 "last"，系统会自动指向最新版本
- 引用失效（记忆中无对应图片）时工具会返回失败，此时再请用户重新发图
- 调用前可以用角色口吻告诉用户你在改图
- 失败时不要重试本工具，用角色口吻告知稍后再试
- 不暴露任何技术细节，保持沉浸感

【耗时】
改图比画图更慢，通常需要十几秒到一分钟，请耐心等待。`,
            parameters: {
                type: "object",
                properties: {
                    prompt: {
                        type: "string",
                        description: "编辑指令，描述要如何修改图片。例如：'把背景换成雪山的日出'、'把照片改成吉卜力动画风格'"
                    },
                    target: {
                        type: "array",
                        items: { type: "string" },
                        minItems: 1,
                        maxItems: 4,
                        description: "待编辑图片数组，每项可以是：图片引用（\"last\"最近一张、\"last-2\"倒数第二张）、图片URL、file_id、或生成图片结果中的local_path。单图编辑传一张；多图合成传多张（最多4张）"
                    },
                    size: {
                        type: "string",
                        description: "输出图片尺寸，不填则跟随原图或使用默认尺寸"
                    }
                },
                required: ["prompt", "target"]
            }
        }
    }
];

export default imageTools;
