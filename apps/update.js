...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        ...
        this.oldCommitId = await this.getcommitId('zhishui-plugin')
        uping = true
        let ret = await this.execSync(command)
        uping = false

        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：止水插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }

        /** 获取插件提交的最新时间 */
        let time = await this.getTime('zhishui-plugin')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`止水插件已经是最新版本\n最后更新时间：${time}`)
        } else {
            await this.reply(`止水插件\n最后更新时间：${time}`)
            this.isUp = true
            let log = await this.getLog('zhishui-plugin')
